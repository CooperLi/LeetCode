...
方法: 
    1.  先把字母 a,b,c,d,...,z 和摩尔斯配对, 这里用的是 trans 方法, 将字母和摩尔斯一一配对; 
        ord 在这里的作用是返回字母的 ascii 值, 比如 ord('a') = 97
        morse[ord(x) - ord('a')] 就保证了每个字母按照顺序对应 morse code
        比如 trans('a') = morse[ord('a') - ord('a')] -> morse[0] -> '.-'
    2.  字母配对后, 把每个词和摩尔斯配对(注意, 这里说的是每个词, 之前 trans 方法是将每个字母配对). 
        这里的 map_to_word 中, 用了 join 这个 string 中自带的方法
        把每个词中已经转换成摩尔斯的字母用''(这里是空的, 所以就是直接连接)连接起来
